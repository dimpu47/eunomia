...
Again, all the components (except operator-sdk v0.10.0 and golint) can be easily installed via Homebrew:
...
The components (except operator-sdk v0.10.0 and golint) can be installed via a distro-specific package manager. You can also download a binary, or install from source.
...
#### Installing operator-sdk v0.10.0 from GitHub release

Unfortunately, operator-sdk version 0.8.1 isn't available via package managers, so you have to install it from the GitHub release as described in the [documentation](https://github.com/operator-framework/operator-sdk/blob/v0.10.0/doc/user/install-operator-sdk.md).
